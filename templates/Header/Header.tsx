...
export const Header = ({ className, ...props }: HeaderProps): JSX.Element => {
	...
	const {
		data: productsByTitle,
		isLoading,
		error,
	} = useGetProductsByTitleQuery(search)
	...
	return (
		<header className={cn(styles.header, className)}>
			<div className={styles.leftSide}>
				<Button type={'header'} onClick={handleActive}>
					Категории
				</Button>
				{isActive ? (
					<div className={cn(styles.dropdown)}>
						<Button type={'text'} onClick={() => handleCategory('fruit')}>
							Фрукты
						</Button>
						<Button type={'text'} onClick={() => handleCategory('vegetable')}>
							Овощи
						</Button>
						<Button type={'text'} onClick={() => handleCategory('meat')}>
							Мясо
						</Button>
						<Button type={'text'} onClick={() => handleCategory('milk')}>
							Молочные продукты
						</Button>
						<Button type={'text'} onClick={() => handleCategory('bread')}>
							Хлебные изделия
						</Button>
					</div>
				) : null}
				...
				{isCategory ? (
					<div className={styles.categoryMenu}>
						{Products.map(product => {
							const filteredData = productsByCategory?.filter(
			...
			<div className={styles.rightSide}>
				...
				/>
				<Link
					href={token ? '/lk/favourites' : '/login'}
				...
				<div className={styles.searchMenu}>
					{search ? (
						<>
							...
							{Products.map(product => {
								const filteredData = productsByTitle?.filter(
									item => item.category === product.type
								)
								if (filteredData && filteredData.length > 0) {
									return (
										...
										>
											...
											<CardRow type={product.type} data={filteredData} />
										</div>
									)
								}
								return null
