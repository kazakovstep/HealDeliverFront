	...
	useEffect(() => {
		switch (content) {
			...
			case 'Заказы':
				...
				break
			case 'Доставщики':
				switchHref('/admin/delivery_people')
				break
			default:
				break
		}
		...
		<Link href={href}>
			...
			>
				{content === 'Главная' ? (
					...
					<HistoryImg className={active ? styles.activeImg : undefined} />
				) : content === 'Избранное' ? (
					<LikeImg className={active ? styles.activeImg : undefined} />
				) : content === 'Корзина' ? (
					<CartImg className={active ? styles.activeImg : undefined} />
				) : content === 'Настройки' ? (
					<SettingsImg className={active ? styles.activeImg : undefined} />
				) : content === 'Пользователи' ? (
					<PersonImg className={active ? styles.activeImg : undefined} />
				) : content === 'Продукты' ? (
					<FoodImg className={active ? styles.activeImg : undefined} />
				) : content === 'Заказы' ? (
					<CartImg className={active ? styles.activeImg : undefined} />
				) : content === 'Доставщики' ? (
					<PersonImg className={active ? styles.activeImg : undefined} />
